...
class MenuSection extends React.Component {
  ...
  render() {
    const {
      navData: { title = "About Us", slug, children: subMenuColumns },
      logoSpace = false,
      justifyContent
    } = this.props;
    ...
    return (
      <>
        ...
        <li
          onMouseEnter={this.onHoverEnterMainItem}
          onMouseLeave={this.onHoverExitMainItem}
          className="menu-item menu-item-type-custom menu-item-object-custom menu-item-has-children wpmega-menu-megamenu wpmega-show-arrow wpmm-menu-align-left wpmega-hide-menu-icon wpmega-horizontal-full-width menu-item-8 menu-item-depth-0 no-dropdown"
          sx={{
        ...
        >
          <Link
            ...
            sx={{
              ...
              "&::after": {
                ...
                transform: megaMenuVisible
                  ? "scaleX(1) translateX(-50%) translateY(0)"
                  : "scaleX(0) translateY(13px)",
          ...
          >
            <span>{title}</span>{" "}
            {megaMenuVisible ? (
              <IoIosArrowUp sx={{ fontSize: 1, position: "relative" }} />
            ) : (
              <IoIosArrowDown sx={{ fontSize: 1, position: "relative" }} />
            )}
          </Link>
